...
#define SPACES "                                "
void my_InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x, int len, char p) {
  rb_red_blk_node* root=tree->root;
  if (x != tree->nil) {
    my_InorderTreePrint(tree,x->right, len + 1, 'r');
    printf("%s%c ", SPACES + strlen(SPACES) - len, p);
    if(x->info) tree->PrintInfo(x->info);
    printf(" "); 
    if(x->key) tree->PrintKey(x->key);
    printf("\n");
    my_InorderTreePrint(tree,x->left, len + 1, 'l');
  }
}

void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
	int len = 0;
	my_InorderTreePrint(tree, x, len, '!');
}
